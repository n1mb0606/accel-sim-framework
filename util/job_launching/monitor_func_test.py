        ...
        if jobStatusCol == None:
            ...
            if len(tokens) > jobStatusCol:
                ...
                if status == "RUNNING" or status == "WAITING_TO_RUN":
                    ...
                    print(
                        "Calling qdel {0}: {1}/{2} ({3})".format(
                            torqueID, app, appArgs, nodeName
                        )
                    )
...
def print_statsfile(options, this_directory):
    ...
    if (
        subprocess.call(
            [
                os.path.join(this_directory, "get_stats.py"),
                "-R",
                "-l",
                options.logfile,
                "-N",
            ...
            stdout=get_stats_out_file,
            stderr=get_stats_out_file,
        )
        != 0
    ):
...
jobstatus_out_filename = os.path.join(
    this_directory, "job_status_out-{0}.txt".format(os.getpid())
)
failed_job_file = ""
...
while True:
    ...
    if (
        subprocess.call(
            [
                os.path.join(this_directory, "job_status.py"),
                "-l",
                options.logfile,
                "-N",
                options.sim_name,
    ...
    print(
        "Passed:{0}/{1}, No error:{2}/{1}, Failed/Error:{3}/{1}, Running:{4}/{1}, Waiting:{5}/{1}".format(
            num_passed, total, num_no_err, num_error, num_running, num_waiting
        )
    )
    if num_error > 0:
